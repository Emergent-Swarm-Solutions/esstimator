...
cmd3="source /opt/ros/$ROS1_DISTRO/setup.bash; rosparam set use_sim_time true; rosbag play $PWD/src/esstimator/test/test2.bag --clock -d 5; exec /bin/bash"

#Command to launch TestCase launch.py
cmd4="source /opt/ros/$ROS2_DISTRO/setup.bash; source $PWD/install/setup.bash; ros2 launch esstimator test_ekf_localization_node_bag2.launch.py; exec /bin/bash"
