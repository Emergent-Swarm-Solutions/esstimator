...
The EKF and UKF can publish tuning telemetry and open a matplotlib window that summarizes all input streams. The visualizer is meant to answer two practical questions:
...
The window auto-launches only when a desktop display is available. If no `DISPLAY` or `WAYLAND_DISPLAY` is present, the filter still publishes the telemetry topics under `tuning/`.
...
## How Streams Are Shown

The visualizer uses the filter's internal input branches, not only the high-level YAML sensor keys.

This matters most for IMUs:

- one IMU can appear as separate inputs for orientation, angular velocity, and linear acceleration
- each branch is treated as its own fused input because it has its own covariance, innovation, and Mahalanobis gate
- the selector and plot titles show both the stream identity and the actual ROS topic name when available

Examples:

- `imu0 orientation (RPY) [ROLL, PITCH, YAW] :: /your/imu/topic`
- `imu0 angular velocity [VROLL, VPITCH, VYAW] :: /your/imu/topic`
- `imu0 linear acceleration [AX, AY, AZ] :: /your/imu/topic`
- `odom0 pose [X, Y, Z, ROLL, PITCH, YAW] :: /your/odom/topic`
...
Each fused input branch gets its own subplot. The blue line is the Mahalanobis distance for each incoming measurement. The orange dashed line is the configured rejection threshold. Red `x` markers are rejected measurements.

Use the selector on the right or click a subplot to inspect it below.

Configured inputs that are not currently producing innovation diagnostics are still shown as gray `[no data]` panels. This is useful when a stream is configured in YAML but is not actually being fused, for example because of covariance flags, disabled update variables, or rejected message content.
...
- Which configured streams are missing entirely from live fusion?
...
For mixed-unit streams such as pose inputs that combine position and orientation, the detail plots split linear/translational and angular/rotational components onto separate y-axes so they are easier to read.
...
- Are the orientation, angular velocity, and acceleration parts of an IMU behaving like separate inputs, as expected?
...
If a configured stream appears as gray `[no data]`:

- verify that the corresponding branch is actually enabled in the update vector
- verify that the incoming message contains that field with a usable covariance
- for IMUs specifically, remember that orientation, angular velocity, and linear acceleration are separate fusion branches
- use the displayed ROS topic name to confirm you are checking the right upstream source

If an IMU branch shows up but not the one you care about:

- `imuX orientation` corresponds to fused `ROLL/PITCH/YAW`
- `imuX angular velocity` corresponds to fused `VROLL/VPITCH/VYAW`
- `imuX linear acceleration` corresponds to fused `AX/AY/AZ`
- tune each branch independently, because each one has its own measurement covariance and rejection threshold
